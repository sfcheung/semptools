...
p2 <- set_edge_label_bg(
        p_pa,
        c("x1~~x2" = "cyan",
          "TX3~~ TX3" = "#00FF00"))
# plot(p2)
p2b <- set_edge_label_bg(
        p_pa,
        c("x2~~x1" = "cyan",
          "TX3~~ TX3" = "#00FF00"))
# plot(p2b)

test_that("set_edge_label_bg", {
  expect_true(p2$graphAttributes$Edges$label.bg[6] !=
                p_pa$graphAttributes$Edges$label.bg[6])
  expect_true(p2$graphAttributes$Edges$label.bg[1] !=
              p_pa$graphAttributes$Edges$label.bg[1])
  expect_true(p2b$graphAttributes$Edges$label.bg[1] !=
              p_pa$graphAttributes$Edges$label.bg[1])
})
