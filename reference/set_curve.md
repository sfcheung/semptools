...
the original one, with curve attributes for selected edges changed. If
...
If `how` is set to `"ratio"`, then the new curve value is equal to the
old curve value multiplied by the supplied value. This mode lets users
increase or decrease the curvature of a curve without knowing the
original value.
