...
Add standard error or confidence interval estimates, in parentheses, to
parameter estimates (edge labels) in a
...
- sep:
  ...
  A character string to separate the coefficient and the standard error
  (in parentheses). Default to " " (one space). Use `"\n"` to enforce a
  line break.

- digits:

  Integer indicating number of decimal places for the appended standard
  errors. Default is 2L.
...
- ests:
  ...
  `rhs`, `se` (for SE), and `ci.lower` and `ci.upper` (for CI). Only
  used when `object` is not specified.
...
- std_type:
  ...
  to compute the standard errors for the standardized solution. Used
  only if standard errors are not supplied directly through `ests`.

## Value

If the input is a
[qgraph::qgraph](https://rdrr.io/pkg/qgraph/man/qgraph.html) object, the
function returns a qgraph based on the original one, with standard error
or confidence interval estimates appended. If the input is a list of
qgraph objects, the function returns a list of the same length.

## Details

Modify a [qgraph::qgraph](https://rdrr.io/pkg/qgraph/man/qgraph.html)
object generated by
[`semPaths`](https://rdrr.io/pkg/semPlot/man/semPaths.html) (currently
in parentheses) to the labels. Require either the original object used
in the semPaths call, or a data frame with the standard error (or
confidence interval) for each parameter. The latter option is for
standard errors/confidence interval not computed by lavaan but by other
functions.
