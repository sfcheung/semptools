...
For example, to change the line width of the path regressing `y` on `x`,
the name should be `"y ~ x"`. To change the line type of the covariance
between `x1` and `x2`, the name should be `"x1 ~~ x2"`. Therefore,
`c("y ~ x1" = 2, "x1 ~~ x2" = 3)` changes the line widths of the path
from `x1` to `y` and the covariance between `x1` and `x2`by 2 and 3,
respectively.
...
values: `from`, `to`, and `new_line_width`. The attribute of the edge
from `from` to `to` will be set to `new_line_width`.
