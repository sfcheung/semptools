...
## Mark all parameter estimates by asterisks based on p-values: `mark_sig()`
...
We know from the
[`lavaan::sem()`](https://rdrr.io/pkg/lavaan/man/sem.html) output that
some paths are significant and some are not. In some disciplines,
...
- The second argument, `object`, is the
  [`lavaan::lavaan()`](https://rdrr.io/pkg/lavaan/man/lavaan.html)
  output used to generate the plot. This output is needed to extract the
  *p*-values.
...
## Add standard error estimates or confidence interval: `mark_se()` and `mark_ci()`
...
- The second argument, `object`, is the
  [`lavaan::sem()`](https://rdrr.io/pkg/lavaan/man/sem.html) output used
  to generate the plot. This output is needed to extract the standard
  errors.

By default, the standard errors are enclosed by parentheses and appended
to the parameter estimates, separated by one space.

The argument `sep` can be used to use another separator. For example, if
`"\n"` is used, the standard errors will be displayed below the
...
## Rotate the residuals: `rotate_resid()` and `safe_resid_position()`
...
- The first argument, `semPaths_plot`, is the
  [`semPlot::semPaths()`](https://rdrr.io/pkg/semPlot/man/semPaths.html)
  output.

- The second argument, `rotate_resid_list`, is the vector to specify how
  the residuals should be rotated. The name is the node for which the
  residual will be rotated, and the value is the degree of rotation.

For example, to achieve the results described above, the vector is
`c(x3 = 135, x4 = 180, x2 = -90)`:

``` r

library(semptools)
my_rotate_resid_list <- c(x3 = 135,
                          x4 = 180,
                          x2 = -90)
p_pa3 <- rotate_resid(
  p_pa,
...
named list are still supported but not recommended. Please see
[`?rotate_resid`](https://sfcheung.github.io/semptools/reference/rotate_resid.md)
on how to use lists of named list.)

If we just want to try preventing overlapping in labels, we can use
[`safe_resid_position()`](https://sfcheung.github.io/semptools/reference/safe_resid_position.md),
which will try to rotate residuals automatically:

``` r

p_pa4 <- safe_resid_position(p_pa)
plot(p_pa4)
```

![](semptools_files/figure-html/unnamed-chunk-13-1.png)

## Set the curve attributes: `set_curve()`
...
- The second argument, `curve_list`, is the vector or list to specify
  the new curvature of the selected arrows.
...
`c("x2 ~~ x1" = -2, "x4 ~ x1" = 1)`, as shown below:
...
![](semptools_files/figure-html/unnamed-chunk-16-1.png)
...
## Set the positions of parameters: `set_edge_label_position()`
...
- The second argument, `position_list`, is the vector or list to specify
  the new position of the selected arrows.

We can use a named vector to specify the changes. The “name” of each
element is of the same form as `lhs-op-rhs` as in
[`lavaan::lavaan()`](https://rdrr.io/pkg/lavaan/man/lavaan.html) model
syntax. In `lavaan`, `y ~ x` denotes an arrow from `x` to `y`.
Therefore, if we want to set the position of the parameter of path
*from* `x` *to* `y` to .25, then the element is `"y ~ x" = .25`. Note
that whether `~` or `~~` is used does not matter.

Therefore, the changes described above can be specified by
`c("x3 ~~ x1" = .25, "x3 ~ x2" = .25, "x4 ~ x1" = .7)`, as shown below:
...
generated by
[`semPlot::semPaths()`](https://rdrr.io/pkg/semPlot/man/semPaths.html),
and change the node labels in the *last step* to create the final plot.
...
![](semptools_files/figure-html/unnamed-chunk-20-2.png)
...
![](semptools_files/figure-html/unnamed-chunk-21-1.png)
...
![](semptools_files/figure-html/unnamed-chunk-23-1.png)
...
p_pa3 <- p_pa |>
  set_curve(c("x2 ~ x1" = -2)) |>
  rotate_resid(c(x1 = 0, x2 = 180, x3 = 140, x4 = 140)) |>
  mark_sig(fit_pa) |>
  mark_se(fit_pa, sep = "\n") |>
  set_edge_label_position(c("x4 ~ x1" = .65)) |>
  change_node_label(c(x1 = "Attitude",
                      x2 = "SbjNorm",
                      x3 = "Intention",
                      x4 = "Behavior"))
plot(p_pa3)
