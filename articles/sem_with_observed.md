...
with latent variables and one or more observed exogenous variables
...
This article assumes basic knowledge in using `semptools` and
[`set_sem_layout()`](https://sfcheung.github.io/semptools/reference/set_sem_layout.md)
(see
...
The following model is to be fitted to the dataset `sem_example`,
provided in the package `semptools`. It is fitted by
...
In this model, `x04` and `x05` are exogenous observed variables. They
affect `f3` and `f4` but they themselves are not indicators of latent
factors.

## Setting `indicator_order` and `indicator_factor`

For a model in which all observed variables are indicators of latent
factors, they are placed in the vector for `indicator_order`.

If an observed variable is not an indicator, then this is how to include
it in the call to
[`set_sem_layout()`](https://sfcheung.github.io/semptools/reference/set_sem_layout.md):

- It should be placed in `indicator_order` and `indicator_factor` as if
  it were a latent factor and it were its own indicator.

- In the matrix for `factor_layout`, set its position as if it were a
  latent factor.

- No need to specify the position of its indicators in the matrix for
  `factor_point_to`. Just leave the cell `NA`.
...
These are the vectors for `indicator_order` and `indicator_factor`. Note
...
This is the matrix for `factor_layout`, created by
...
This is the matrix for `factor_point_to`, created by
[`auto_factor_point_to()`](https://sfcheung.github.io/semptools/reference/auto_factor_point_to.md)
(added in `semptools` 0.2.9.15) \[^If not available, the matrix can be
...
![](sem_with_observed_files/figure-html/unnamed-chunk-8-1.png)
...
please refer to
[`vignette("quick_start_sem")`](https://sfcheung.github.io/semptools/articles/quick_start_sem.md).
For illustrations on how to modify a path diagram from
[`semPlot::semPaths()`](https://rdrr.io/pkg/semPlot/man/semPaths.html)
using functions in `semptools`, please refer to
[`vignette("semptools")`](https://sfcheung.github.io/semptools/articles/semptools.md).
