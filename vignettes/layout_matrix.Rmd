...
One useful feature of `semPlot::semPaths` is using a layout matrix to specify
the location each node in a structural equation model, by setting the argument
`layout`. This guide briefly explains how to use `layout`, and then introduces
...
If we fit the model by `lavaan` and use `semPlot::semPaths`,
without using `layout`, this is the resulting plot:
...
This layout is different the convention used in psychology.
...
```{r grid, echo = FALSE}
plot(p_pa2con)
segments(-1.25, 1.25, 1.25, 1.25, col = "red", lwd = 4)
segments(-1.25, .5, 1.25, .5, col = "red", lwd = 4)
segments(-1.25, -.5, 1.25, -.5, col = "red", lwd = 4)
segments(-1.25, -1.25, 1.25, -1.25, col = "red", lwd = 4)
segments(-1.25, -1.25, -1.25, 1.25, col = "red", lwd = 4)
segments(-.625, -1.25, -.625, 1.25, col = "red", lwd = 4)
segments(0, -1.25, 0, 1.25, col = "red", lwd = 4)
segments(.625, -1.25, .625, 1.25, col = "red", lwd = 4)
segments(1.25, -1.25, 1.25, 1.25, col = "red", lwd = 4)
```

We then create a matrix of the same dimension as the
grid, and initialize the cells by `NA`, denoted a cell
with nothing.
...
(node) by setting the corresponding cell to the name
...
The what-you-see-is-what-you-get approach in the previous
...
find the desirable layout. The `layout_matrix` function
...
of deleting the 3rd rows of `NA`, we can use 
...
The `factor_layout` argument in `set_sem_layout` works
like `layout` in `semPlot::semPaths`. However, if we
use `layout` for a structural models with latent factors,
we need to specify the positions of all nodes, that is,
all the indicators and all the factors. In `set_sem_layout`,
...
difference is, only the names of the *factors* need
...
segments(-1.25, -1.25, 1.25, -1.25, col = "red", lwd = 4)
segments(-1.25, -1.25, -1.25, 1.25, col = "red", lwd = 4)
segments(-1.25 + 2.5/3, -1.25, -1.25 + 2.5/3, 1.25, col = "red", lwd = 4)
segments(-1.25 + 2 * 2.5/3, -1.25, -1.25 + 2 * 2.5/3, 1.25, col = "red", lwd = 4)
segments(1.25, -1.25, 1.25, 1.25, col = "red", lwd = 4)
```
