...
#'
#'@description Configure the layout of factors and adjust other aspects of an
#'             SEM graph by [semPlot::semPaths].
#'
#'@details Modified a [qgraph::qgraph] object generated by semPaths based on an
#' SEM model with latent factors. Currently only support a model in which all
#' exogenous variables are latent factors, and all observed variables
#' are
...
#'
#'@return
#' A [qgraph::qgraph] based on the original one, with various aspects of the model
#' modified.
#'
#'@param semPaths_plot A [qgraph::qgraph] object generated by semPaths, or a
#' similar qgraph object modified by other [semptools] functions.
#'@param indicator_order A string vector of the indicators. The order of the
#'                       names
...
#'                       load on f1, then vector should be either
#'                        c("x1", "x2", "x4", "x3", "x5", "x6")
#'                       or c("x3", "x5", "x6", "x1", "x2", "x4"). Indicators
#'                        within
#'                       a group can be ordered in any way.
#'@param indicator_factor A string vector of the same length of the indicator
#'                        order,
#'                        storing the name of the factor for which each of the
#'                        indicator
#"                        in `indicator_factor` loads on.
#'                       For example, if x1, x2, x4 load on f2, and x3, x5, x6
#'                       load on f1, and indicator_order is
#'                        c("x3", "x5", "x6", "x1", "x2", "x4"),
#'                       then `indicator_factor` should be
#'                        c("f2", "f2", "f2", "f1", "f1", "f1").
#'@param factor_layout A matrix of arbitrary size. This matrix will serve as a
#'                     grid for 
#'                     users to specify where each latent factor should be
#'                     placed 
#'                     approximately on the graph. Each cell should contain NA
#'                     or
#'                     the name of a latent factor. The locations of all latent
#'                     factors
#'                     must be explicitly specified by this matrix.
#'@param factor_point_to A matrix of the same size as `factor_layout`. This matrix
#'                       specifies where the indicators of each factor are
#'                       positioned.
#'                     Each cell should contain NA or one of these strings:
#'                     "down", "left", "up", or "right". This is the direction
#'                     that the
...
#' positions of the indicators of selected latent factors.
#' It can be named vector or a list of named lists.
#' For a named vector, The name is the factor of which the indicators will
#' be "pushed", and the value is how "hard" the push is: the multiplier
#' to the distance from the factor to the indicators. If this value is 1, then
#' there is no change. If this value is greater than 1, then the indicators are
#' pushed away from the latent factor. If this value is less than 1, then
#' the indicators are pulled toward the latent factor. For example, to push the
#' indicators of `f3` away from `f3`, and pull the indicators of `f4` toward
#' `f4`, the argument can be set to `c(f3 = 1.5, f4 = .5)`.
#' For a list of named list, each named list has two named elements:
#' `node`, the name of a latent factor, and `push`, how the positions of
#' its indicators will be adjusted. For example, to have the same effect as
#' the vector above, the list is `list(list(node = "f3", push = 1.5),
#' list(node = "f4", push = .5))`. 
#'@param indicator_spread (Optional) This argument is used to adjust the
#' distance between
#' indicators of selected latent factors. It can be a named vector or
#' a list of named lists.
#' For a named vector, the name is the factor of which the indicators
#' will be spread out. The value is the multiplier to the distance between
#' neighboring indicators.
#' If this value is equal to 1, there is no change. Larger than
#' one,
#' the indicators will be "spread" away from each other.
#' Less than one, the indicators will be placed closer to
#' each
#' others.
#' For example, to spread the indicators of `f1` and `f4`
#' farther away from each other, this argument can be set to 
#' `c(f1 = 2, f4 = 1.5)`, with the indicators of `f1` being spread out more
#' than those of `f4`. 
#' For a list of named list, each named list has two named elements:
#' `node`, the name of a latent factor, and `spread`, how the distance 
#' between indicators will be
#' adjusted. For example, to have the same effect as the vector above, the
#' argument can be set to `list(list(node = "f1", spread = 2), list(node = "f4",
#' spread = 1.5))`.
#'@param loading_position (Optional) Default is .5.
#' This is used adjust the position of
#' the loadings.
#' If this is one single number, it will be used to set the
#' positions
#' of all loadings. If it is .5, the loadings are placed on
#' the center of
#' the arrows. Larger the number, closer the loadings to the
#' indicators.
#' Smaller the number, closer to the latent factors.
#' This argument also accepts a named vector or a list of named lists,
#' allowing users
#' to
#' specify the positions of loadings for each factor
#' separately.
#' For a named vector, in each element, the name is the factor whose
#' loadings will be moved. The value is the positions of its loadings. 
#' The default is .50. We only need to specify the positions for factors to be
#' changed from .50 to other values. For example, move the loadings of `f2`
#' closer to the indicators and those of `f4` close to the `f4`, this argument
#' can be set to `c(f2 = .7, f4 = .3)`.
#' For a list of named list, each named list should have two named elements:
#' `node`, the name of the 
#' latent factor, and `position`, the positions of all loadings
#' of this 
#' factors. To have the same effect as the vector above, this list can be used:
