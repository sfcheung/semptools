...
#'
#'@details
#'These functions can be used to edit the nodes in an [`semPlot::semPlotModel`] generated 
#'by [semPlot::semPlotModel()]. The edited object can then be passed 
#'to [semPlot::semPaths()] to generate a path diagram.
#'
#'Use [keep_nodes()] to specify the nodes to be kept. All other nodes will be 
#'removed.
#'
#'Use [drop_nodes()] to specify the nodes to be dropped. All other nodes will be 
#'kept.
#'
#'@return
#' An object of the class [`semPlot::semPlotModel`].
#' 
#'@param object An an [`semPlot::semPlotModel`] generated by [semPlot::semPlotModel()].
...
drop_nodes <- function(object, nodes) {
    ...
    tmp[, "par"] <- seq_len(nrow(tmp))
    object@Pars <- tmp
    tmp <- object@Vars
    pos_to_keep <- !(tmp[, "name"] %in% nodes)
    object@Vars <- object@Vars[pos_to_keep, ]
    object@ObsCovs[[1]] <- object@ObsCovs[[1]][pos_to_keep, pos_to_keep]
    object@ImpCovs[[1]] <- object@ImpCovs[[1]][pos_to_keep, pos_to_keep]
    object
  }
