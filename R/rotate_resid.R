...
rotate_resid <- function(
  ...
  # Fix the names
  rotate_resid_list_fixed <- to_new_value(
                        rotate_resid_list,
                        original_name = "rotate"
                      )

  # Convert to radian
  if (is.list(rotate_resid_list_fixed)) {
    for (i in seq_along(rotate_resid_list_fixed)) {
      rotate_resid_list_fixed[[i]]$new_value <-
        rotate_resid_list_fixed[[i]]$new_value * pi / 180
    }
  } else {
    rotate_resid_list_fixed <-
      rotate_resid_list_fixed * pi / 180
  }
