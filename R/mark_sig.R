...
#'
#'@description Mark parameter estimates (edge labels) based on p-value.
#'
#'@details Modified a qgraph object generated by semPaths and add marks
#'         (currently asterisk, "*") to the labels based on their p-values.
#'         Require the original object used in the semPaths call.
#'
...
#'
#'@return
#' A qgrpah based on the original one, with marks appended to edge labels based on
#'  their p-values.
#' 
...
#'@param object The object used by semPaths to generate the plot. Use the same argument 
#'                name used in semPaths to make the meaning of this argument obvious. 
#'                Currently only object of class \linkS4class{lavaan} is supported.
#'@param alphas A named numeric vector. Each element is the cutoff (level of 
#'                significance), and the name of it is the symbol to be used if
#'                p-value is less than this cutoff. The default is 
#'                c("*" = .05, "**" = .01, "***" = .001).
#'
#'@examples
#'mod_pa <- 
