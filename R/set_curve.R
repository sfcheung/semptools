...
#' [semPlot::semPaths] and change the curve attributes of
#' selected edges.
#'
#'@return
#' A [qgraph::qgraph] based on the original one, with curve attributes
#' for selected edges changed.
#'
#'@param semPaths_plot A [qgraph::qgraph] object generated by
#' [semPlot::semPaths], or a similar qgrpah
...
#' of the path from `x1` to `y` and the covariance between `x1` and `x2`
#' to -3 and 2, respectively. The order of the
#' two nodes *may* matter for covariances. Therefore, if the curve
#' of a covariance is not changed, try switching the order of the two
#' nodes.
#' For a list of named lists, each named list should have three
#' named values: `from`, `to`, and `new_curve`. The curve attribute
#' of the edge from `from` to `to` will be set to `new_curve`.
#'
#'@examples
#'mod_pa <-
#'  'x1 ~~ x2
